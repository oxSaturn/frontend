...
import {
  ...
  useOFlowDiscount,
  useOFlowExercise,
  useOFlowGetDiscountedPrice,
  useOFlowGetTimeWeightedAveragePrice,
  usePrepareOFlowExercise,
...
export function Redeem() {
  ...
  const {
    ...
    address,
    chainId: pulsechain.id,
    token: W_NATIVE_ADDRESS as `0x${string}`,
  });

  const { data: oFlowBalance, refetch: refetchOFlowBalance } =
    useOFlowBalanceOf({
      chainId: pulsechain.id,
      args: [address!],
      enabled: !!address,
  ...
  const { data: blotrPrice } = useOFlowGetTimeWeightedAveragePrice({
    chainId: pulsechain.id,
    args: [parseEther("1")],
    select: (data) => formatEther(data),
  });

  const { data: discountedPrice } = useOFlowGetDiscountedPrice({
    chainId: pulsechain.id,
    args: [parseEther("1")],
    select: (data) => formatEther(data),
  });
  ...
  const { config: approveConfig } = usePrepareWplsApprove({
    chainId: pulsechain.id,
    args: [
  ...
  const { config: exerciseBlotrConfig } = usePrepareOFlowExercise({
    chainId: pulsechain.id,
    args: [
