...
export const getPairsWithGauges = async (
  ...
  const ps2 = pairsWithkilledGauges.map((pair, i) => {
    ...
    pair.gauge = {
      ...
      min_apr: pair.killed_gauges[0].min_apr,
      max_apr: pair.killed_gauges[0].max_apr,
      median_tbv: pair.killed_gauges[0].median_tbv,
      min_tbv: pair.killed_gauges[0].min_tbv,
      max_tbv: pair.killed_gauges[0].max_tbv,
...
export const checkNFTActionEpoch = async (
  ...
  // 7 days epoch length
  const votedInCurrentEpoch = lastVoted > nextEpochTimestamp - 7 * 24 * 60 * 60;
